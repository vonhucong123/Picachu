...
void BoardView::showBoard()
{
	...
	squareSize = visibleSize.width / ((board->getNColumns() + 2))*0.5;
...
Sprite* BoardView::addPokemon(int row, int column, int type)
{
	...
	listener->setSwallowTouches(true);		// sự kiện bị nuốt
...
bool BoardView::removePokemon(int row, int column)
{
	...
	pokemons[row][column]->setVisible(false);
